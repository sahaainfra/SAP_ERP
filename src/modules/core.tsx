    ...
    <div className="inline-flex items-center gap-0.5 bg-canvas border border-line rounded-lg p-0.5 overflow-x-auto max-w-full">
      {options.map((o) => (
        <button key={o.k} onClick={() => onChange(o.k)}
          className={cx("px-3 h-8 rounded-md text-[12px] font-semibold transition-all active:scale-[0.98] whitespace-nowrap flex items-center gap-1.5",
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-3">
      ...
      <div className="relative">
        ...
        {openSf && (
          <>
            <div className="fixed inset-0 z-20" onClick={() => setOpenSf(false)} />
...
export function DataTable({ pageKey, rows, cols, idKey = "id", pageSize = 8, selectable, bulkActions, onRow, empty }: {
  ...
  return (
    <div>
      {selectable && sel.length > 0 && (
        <div className="flex items-center gap-2 mb-2 px-3 h-9 rounded-lg bg-brand-50 border border-brand-200 fade-up">
          ...
          <div className="ml-auto flex gap-1.5">
            {bulkActions?.map((b) => <Btn key={b.label} sm onClick={() => { b.on(sel); setSel([]); }}>{b.label}</Btn>)}
              ...
              <tr className="text-[10.5px] uppercase tracking-[0.08em] text-ink-400">
                {selectable && (
                  <th className="w-8 pb-2 pr-2">
                    <input type="checkbox" className="h-3.5 w-3.5 accent-[#0c7264] cursor-pointer"
                      checked={view.length > 0 && view.every((r) => sel.includes(r[idKey]))}
                ...
                {visCols.map((c) => (
                  <th key={c.key} style={{ width: c.w }}
                    className={cx("font-bold pb-2 pr-3 whitespace-nowrap cursor-pointer select-none hover:text-ink-700 transition-colors", c.align === "right" && "text-right", c.align === "center" && "text-center")}
                    onClick={() => setSort((st) => (st?.k === c.key ? { k: c.key, d: st.d === 1 ? -1 : 1 } : { k: c.key, d: c.align === "right" ? -1 : 1 }))}>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-3 mt-1 border-t border-line/80">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <Btn sm onClick={() => { if (sorted.length) { exportCSV("meridian-" + pageKey, visCols, sorted); toast("success", `Exported ${sorted.length} rows to Excel (CSV)`); } }} disabled={!sorted.length}><IDownload size={12} /> Excel</Btn>
          <Btn sm onClick={doPrint} disabled={!sorted.length}><IDownload size={12} /> PDF / Print</Btn>
...
export function Modal({ open, onClose, title, children, footer, wide }: { open: boolean; onClose: () => void; title: string; children: ReactNode; footer?: ReactNode; wide?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4">
      <div className="absolute inset-0 bg-side-900/45 backdrop-blur-[2px]" onClick={onClose} />
      <div className={cx("relative w-full bg-surface border border-line rounded-xl shadow-pop p-5 fade-up max-h-[90vh] overflow-y-auto", wide ? "max-w-[760px]" : "max-w-[480px]")}>
...
export function AddBtn({ label, onClick, disabled, tip }: { label: string; onClick: () => void; disabled?: boolean; tip?: string }) {
  return (
    <span className={cx("relative", disabled && "tip cursor-not-allowed")} data-tip={disabled ? tip : undefined}>
      <Btn kind="primary" onClick={onClick} disabled={disabled}>
        <IPlus size={13} /> {label}
      </Btn>
    </span>
  );
...
export function TrafficLight({ status }: { status: "green" | "amber" | "red" }) {
  return <span className={cx("inline-block h-2.5 w-2.5 rounded-full shrink-0", status === "green" ? "tl-green" : status === "amber" ? "tl-amber" : "tl-red")} />;
}
