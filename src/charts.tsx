/* Meridian ERP · chart components */
import {
  ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend,
  AreaChart, Area, LineChart, Line, ComposedChart, PieChart, Pie, Cell,
} from "recharts";
import { AGING, BUDGET_ACTUAL, CASHFLOW, PLANNED_ACTUAL, REV_EXP } from "./data";
...
export function CashFlowChart({ range }: { range: string }) {
  const data = sliceByRange(CASHFLOW.map((c) => ({ ...c, net: +(c.inflow - c.outflow).toFixed(1) })), range);
  return (
    <ResponsiveContainer width="100%" height={228}>
